...
class SQLModID(Base):
    ...
    workshop_id = Column(String, ForeignKey(
        "mods.workshop_id"), primary_key=True)
...
def sync(mods: list[Mod]):
    ...
    for mod in mods:
        existing_mod = session.query(SQLMod).filter_by(
            workshop_id=mod.workshop_id).first()
        if existing_mod:
            existing_mod.thumbnail = mod.thumbnail
            existing_mod.name = mod.name
            for mod_id in mod.mod_ids:
                existing_mod_id = session.query(SQLModID).filter_by(
                    workshop_id=mod.workshop_id, mod_id=mod_id.id).first()
                if existing_mod_id:
...
def update_enabled(workshop_id: str, mod_id: str, enabled: bool):
    ...
    session = session_class()
    existing_mod_id = session.query(SQLModID).filter_by(
        workshop_id=workshop_id, mod_id=mod_id).first()
    if existing_mod_id:
