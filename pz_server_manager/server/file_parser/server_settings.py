...
class ServerSettings():
    ...
    def load(self):
        """load"""
        with open(self.file_path, "r", encoding="utf-8") as configfile:
            self._file_lines = configfile.readlines()
            self._mods_index, self._workshop_index = find_mods_indices(
                self._file_lines)
            if self._mods_index == -1 or self._workshop_index == -1:
                raise ValueError(
                    "Could not determin line number for replacing the mods.")
